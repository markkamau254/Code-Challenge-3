...
const API_KEY = 'api_key=4138ba79e79b5e838c1fcbe5dc11a86f';
...
function getMovies(url) {
    ...
    fetch(url).then(res => res.json()).then(data => {
        ...
        if(data.results.length !== 0){
            ...
            current2.innerText = currentPage;
            ...
            bdy.innerHTML= `<h1 class="no-results">No Results Found!</h1>`
        }
...
function showMovies(data) {
    ...
    data.forEach(box1 => {
        const {title, poster_path, overview, id} = box1;
        const movieEl = document.createElement('div');
        movieEl.classList.add('box1');
        movieEl.innerHTML = `
       ...
       <div class="boxcontent">
        ...
        <div class="tooltip">
            Overview 
            <div class="tooltiptext">${overview}</div>
            <br/>
            <button class="know-more" id="${id}">See More</button>
                    ...
                    <div class="rating">
                    <input value="5" name="rate${id}" id="star5-${id}" type="radio">
                    <label title="text" for="star5-${id}"></label>
                    <input value="4" name="rate${id}" id="star4-${id}" type="radio" checked="">
                    <label title="text" for="star4-${id}"></label>
                    <input value="3" name="rate${id}" id="star3-${id}" type="radio">
                    <label title="text" for="star3-${id}"></label>
                    <input value="2" name="rate${id}" id="star2-${id}" type="radio">
                    <label title="text" for="star2-${id}"></label>
                    <input value="1" name="rate${id}" id="star1-${id}" type="radio">
                    <label title="text" for="star1-${id}"></label>
                    </div>                    

                        
                        <button id="butt">
                            <a href="https://numetro.co.za/">Book Ticket</a>
                            <div class="icon-1">
                            <svg
        ...
        `
                ...
                movieEl.querySelectorAll('.rating input').forEach(input => {
                  input.addEventListener('change', (event) => {
                      const rating = event.target.value;
                      console.log(`Movie ID: ${id}, Rating: ${rating}`);
                      // Handle rating change (e.g., save rating to server)
                  });
                });
      


        bdy.appendChild(movieEl);
      ...
      if(videoData.results.length > 0){
        ...
        videoData.results.forEach((video, idx) => {
          ...
          if(site == 'YouTube'){
            ...
            embed.push(`
              <iframe width="760" height="515" src="https://www.youtube.com/embed/${key}" title="${name}" class="embed hide" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
// fetch('https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=4138ba79e79b5e838c1fcbe5dc11a86f')
//     .then(response => {
